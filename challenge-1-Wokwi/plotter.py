...
def parse(file_list):
    ...
    for file, ranges in file_list:
        with open(file, 'r') as f:
            ...
            tmp_groups = [{'name': range['name'], 'values': [], 'range': range['range']} for range in ranges]
            for value in values:
    ...
    table = [
        [group['name'], group['avg']] for group in groups
    ]

    print(tabulate(table, headers=['State', 'Average Power Consumption (mW)'], tablefmt='github'))

if __name__ == '__main__':
    files = [
        [
            './instructions/deep_sleep.csv', 
            [
        ...
        [
            './instructions/transmission_power.csv', 
            [
